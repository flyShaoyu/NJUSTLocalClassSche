...
class ExamNotificationService : Service() {
  ...
  private fun buildOngoingNotification(exam: ExamOccurrence): Notification {
    ...
    if (now.isBefore(exam.startAt)) {
      val minutesLeft = Duration.between(now, exam.startAt).toMinutes().coerceAtLeast(0)
      title = "即将考试：${exam.exam.courseName}"
      text = "$room 座位号 $seat · ${formatExamStartTime(exam)} · 还有${minutesLeft}分钟开考"
    } else {
      title = "正在考试：${exam.exam.courseName}"
      text = "$room 座位号 $seat · ${exam.exam.examTime}"
    }
  ...
  companion object {
    private const val ONGOING_CHANNEL_ID = "classsche_exam_ongoing_v1"
    private const val ONGOING_NOTIFICATION_ID = 2101
  }
}
