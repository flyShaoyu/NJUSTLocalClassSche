...
class LocalScoreEditorActivity : AppCompatActivity() {
  ...
  private fun saveCurrentEntry() {
    ...
    persistScores(pendingItem = item)
  ...
  private fun persistScores(pendingItem: JSONObject? = null) {
    val array = JSONArray()
    var storedPendingItem: JSONObject? = null
    scores.forEachIndexed { index, item ->
      val normalized = JSONObject(item.toString())
      normalized.put("index", index + 1)
      array.put(normalized)
      if (pendingItem != null &&
        normalized.optString("courseName") == pendingItem.optString("courseName") &&
        normalized.optString("semester") == pendingItem.optString("semester") &&
        normalized.optString("score") == pendingItem.optString("score") &&
        normalized.optString("courseCode") == pendingItem.optString("courseCode")
      ) {
        storedPendingItem = normalized
      }
    }
    File(filesDir, SCORE_JSON_FILE).writeText(array.toString(), Charsets.UTF_8)
    val meta = if (storedPendingItem != null) {
      JSONObject().apply {
        put("pending", true)
        put("items", JSONArray().put(buildScoreUpdateMetaItem(storedPendingItem!!)))
      }
    } else {
      JSONObject().apply {
        put("pending", false)
  ...
  private fun buildScoreUpdateMetaItem(item: JSONObject): JSONObject {
